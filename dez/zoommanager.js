...
UI.loadEnd = function () {
	...
	var cv = document.getElementById('rendering-canvas');
	var link = document.createElement('a');
	link.download = 'filename.png';
	link.href = cv.toDataURL()
	link.click();
		/*
		CanvasToTIFF.toObjectURL(cv, function(url) {
			// can be used as source for image or download target
			console.log('converted');
			downloadURL(url,"sample.zif");
		});*/
};
...
ZoomManager.init = function () {
	...
	if (!ZoomManager.proxy_url) ZoomManager.proxy_url = "https://ophir.alwaysdata.net/dezoomify/proxy.php";
	ZoomManager.status = {
		"error": false,
		"loaded": 0,
		"totalTiles": 1
